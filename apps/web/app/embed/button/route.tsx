...
const minifiedJs = 'function run(e,t,n,i,a){if(!("IdentityCredential"in window))return;let r=!1;try{navigator.credentials.get({identity:Object.defineProperty({},"mode",{get(){r=!0}})}).catch(()=>{})}catch(e){}let o=document.getElementById("f");function d(a){return navigator.credentials.get({mediation:"optional",identity:{providers:[{...e,nonce:`${i}:${n}`,fields:[t.includes("indentify")&&"name",t.includes("email")&&"email"].filter(Boolean),params:{scope:t,code_challenge:n,code_challenge_method:i}}],mode:a}})}IdentityProvider.getUserInfo(e).then(e=>{e&&e.length>0&&(document.getElementById("t").innerText="Sign in as "+e[0].name)}),o.addEventListener("submit",e=>{r&&(e.preventDefault(),parent.postMessage({type:"gw2.me:pkce-required"}, "*"))}),parent.postMessage({type:"gw2.me:pkce-required"}, "*"),window.addEventListener("message",e=>{parent&&e.source===parent&&"object"==typeof e.data&&"type"in e.data&&"gw2.me:pkce"===e.data.type&&d("active").then(console.log)}),d("passive").then(e=>{e&&"token"in e&&open(a.toString()+"&code="+e.token,"_top")})}';

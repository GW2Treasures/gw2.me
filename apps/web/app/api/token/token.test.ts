...
import { Account, Authorization, Client, ClientSecret } from '@gw2me/database';
import { expiresAt } from '@/lib/date';
import { Scope } from '@gw2me/client';
import { describe, expect, it } from '@jest/globals';

type MockAuthorization = Authorization & { client: Pick<Client, 'type'> & { secrets: Pick<ClientSecret, 'secret'>[] }, accounts: Pick<Account, 'id'>[] };
  ...
  describe('errors', () => {
    ...
    describe('authorization_code', () => {
      ...
      it('Missing client secret', async () => {
        dbMock.authorization.findUnique.mockResolvedValue({ ...mockAuthorization, client: { type: 'Confidential', secrets: [{ secret: 'secret' }] }} as MockAuthorization);

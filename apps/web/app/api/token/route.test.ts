...
describe('/api/token', () => {
  describe('errors', () => {
    ...
    it('Missing grant_type', () => expect(handleTokenRequest({ client_id: 'test' })).rejects.toBeOAuth2Error(OAuth2ErrorCode.invalid_request, 'Invalid grant_type'));
    it('Invalid grant_type', () => expect(handleTokenRequest({ client_id: 'test', grant_type: 'foo' })).rejects.toBeOAuth2Error(OAuth2ErrorCode.invalid_request, 'Invalid grant_type'));
